...
def test_create_tables(db):
    db.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='documents'")
    assert db.cursor.fetchone() is not None

    db.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='conversations'")
    assert db.cursor.fetchone() is not None
...
def test_push_conversation_log_record(db):
    ...
    assert db.push_conversation_log_record(question, answer, timestamp, conversation_id) is True

    db.cursor.execute("SELECT question, answer, timestamp, conversation_id FROM conversations WHERE conversation_id = ?", (conversation_id,))
